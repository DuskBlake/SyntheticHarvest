...
static var day_count:int = 0

var pass_day:bool = false
...
func  _process(delta: float) -> void:
	...
	if animation_player.current_animation:
		...
		if animation_player.current_animation_position > 14 && animation_player.current_animation_position < 15 && !pass_day:
			day_count+=1
			pass_day = true
			print(day_count)
	if !extractor:
